            ...
            <div style={{ display: "grid", gap: 16 }}>
              {patientUpcoming.length === 0 ? (
                ...
                patientUpcoming.map((appt) => {
                  ...
                  return (
                    <div key={appt.id || appt._id} className="glass-card" style={{ padding: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => rescheduleAppointment(appt.id || appt._id)} className="btn-secondary" style={{ padding: "8px 16px", fontSize: "0.85rem" }}>
                          Reschedule
                        </button>
                        <button onClick={() => cancelAppointment(appt.id || appt._id)} className="btn-danger" style={{ padding: "8px 16px", fontSize: "0.85rem" }}>
                          Cancel
                        </button>
                      </div>
                    </div>
              ...
              {patientPast.length === 0 ? (
                ...
                patientPast.map((appt) => {
                  ...
                  const existingReview = doctorReviews.find((review) => {
                    ...
                    return reviewDoctorId === appt.doctorId;
                  });

                  return (
                    <div key={appt.id || appt._id} className="glass-card" style={{ padding: 20 }}>
                      ...
                      <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid rgba(0, 217, 255, 0.1)" }}>
                        {existingReview ? (
                          ...
                          <ReviewForm doctorId={appt.doctorId} submitReview={submitReview} />
                        )}
                      </div>
                    </div>
                  );
...
function ReviewForm({ doctorId, submitReview }) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ marginBottom: 12 }}>
        <label style={{ display: "block", color: "var(--text-secondary)", marginBottom: 6 }}>
          Rating
        </label>
        <select
          value={rating}
