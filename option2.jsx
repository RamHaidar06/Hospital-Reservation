...
export default function App() {
    ...
    const doctors = [
      {
        ...
        licenseNumber: "MED-92341",
        yearsExperience: 9,
        bio: "Pediatrician focused on preventive care and parent guidance.",
        phone: "",
      ...
      },
      {
        ...
        firstName: "Karim",
        lastName: "Nassar",
        email: "karim@medicare.io",
        password: "123456",
        specialty: "cardiology",
        licenseNumber: "MED-44120",
        yearsExperience: 14,
        bio: "Cardiologist with a focus on hypertension and preventive cardiology.",
        phone: "",
  ...
  // --- Modals (edit profile, doctor details, etc.) ---
  ...
  // --- Forms state (patient login/register/forgot) ---
  ...
  const [doctorEditForm, setDoctorEditForm] = useState({
    firstName: "", lastName: "", email: "", specialty: "", license: "", experience: "", bio: "",
  });

  // --- Book appointment form inside doctor detail modal ---
  const [apptForm, setApptForm] = useState({ date: "", time: "", reason: "", notes: "" });

  // --- Doctor availability editing (working days + time) ---
  ...
  const currentDoctor = useMemo(() => {
    ...
    return allData.doctors.find((d) => d.id === loggedInDoctor.id) || loggedInDoctor;
  }, [loggedInDoctor, allData.doctors]);

  const currentPatient = useMemo(() => {
    if (!loggedInPatient) return null;
    return allData.patients.find((p) => p.id === loggedInPatient.id) || loggedInPatient;
  ...
  function handlePatientLoginSubmit(e) {
    ...
    const patient = allData.patients.find((p) => p.email === email && p.password === password);
    if (!patient) return showMessage("✗ Invalid email or password", "error");
  ...
  function handlePatientRegisterSubmit(e) {
    e.preventDefault();
    ...
    if (!validateEmail(email)) return showMessage("✗ Invalid email format", "error");
    if (allData.patients.some((p) => p.email === email)) return showMessage("✗ Email already registered", "error");
    ...
    setAllData((d) => ({ ...d, patients: [...d.patients, newPatient] }));
  ...
  function handlePatientForgotSubmit(e) {
    ...
    showMessage("✓ Recovery link sent to " + patientForgotEmail.trim(), "success");
    setPatientForgotEmail("");
    setTimeout(() => setPatientAuthView("login"), 900);
  }

  // --- AUTH: DOCTOR ---
  function handleDoctorLoginSubmit(e) {
    e.preventDefault();
    const email = doctorLogin.email.trim();
    const password = doctorLogin.password;
    if (!validateEmail(email)) return showMessage("✗ Invalid email format", "error");

    const doctor = allData.doctors.find((d) => d.email === email && d.password === password);
    if (!doctor) return showMessage("✗ Invalid email or password", "error");
    ...
    // init drafts from doctor
    const wd = new Set((doctor.workingDays || "").split(",").filter(Boolean));
    setWorkingDaysDraft(wd);
    setStartTimeDraft(doctor.startTime || "09:00");
    setEndTimeDraft(doctor.endTime || "17:00");

    showMessage("✓ Welcome back, Dr. " + doctor.lastName + "!", "success");
  }

  function handleDoctorRegisterSubmit(e) {
    e.preventDefault();
    ...
    if (!firstName || !lastName) return showMessage("✗ First and last name are required", "error");
    if (!validateEmail(email)) return showMessage("✗ Invalid email format", "error");
    if (allData.doctors.some((d) => d.email === email)) return showMessage("✗ Email already registered", "error");
    ...
    setAllData((d) => ({ ...d, doctors: [...d.doctors, newDoctor] }));
  ...
  function handleDoctorForgotSubmit(e) {
    ...
    showMessage("✓ Recovery link sent to " + doctorForgotEmail.trim(), "success");
  ...
  // --- PATIENT DASHBOARD RENDER HELPERS ---
  ...
  const patientPast = useMemo(() => {
    if (!currentPatient) return [];
    return allData.appointments
      .filter((a) => a.patientId === currentPatient.id && new Date(a.appointmentDate) < new Date())
      .sort((a, b) => new Date(b.appointmentDate) - new Date(a.appointmentDate));
  }, [allData.appointments, currentPatient]);

  const visibleDoctors = useMemo(() => allData.doctors.slice(0, 6), [allData.doctors]);

  // --- DOCTOR DASHBOARD RENDER HELPERS ---
  const doctorAppointments = useMemo(() => {
    if (!currentDoctor) return [];
    return allData.appointments.filter((a) => a.doctorId === currentDoctor.id && a.status !== "cancelled");
  ...
  function savePatientProfile(e) {
    ...
    setAllData((d) => ({
      ...d,
      patients: d.patients.map((p) => (p.id === updated.id ? updated : p)),
    }));
  ...
  function saveDoctorProfile(e) {
    ...
    setAllData((d) => ({
      ...d,
      doctors: d.doctors.map((doc) => (doc.id === updated.id ? updated : doc)),
    }));
  ...
  const scheduleOverview = useMemo(() => {
    if (!currentDoctor) return [];
    const workingDays = (currentDoctor.workingDays || "").split(",").filter(Boolean);
    ...
    return Array.from({ length: 7 }).map((_, i) => {
      ...
      const isWorking = workingDays.includes(dayValue);

      const dateISO = date.toISOString().split("T")[0];
      const appts = allData.appointments.filter(
        (a) => a.doctorId === currentDoctor.id && a.appointmentDate === dateISO && a.status !== "cancelled"
      );
      ...
      return {
        ...
        isWorking,
        hours: isWorking ? `${currentDoctor.startTime} - ${currentDoctor.endTime}` : "Day Off",
        count: appts.length,
      };
    });
  }, [allData.appointments, currentDoctor]);

  // --- PATIENT: DOCTOR DETAILS + BOOKING ---
  ...
  const selectedDoctor = useMemo(() => {
    ...
    return allData.doctors.find((d) => d.id === selectedDoctorId) || null;
  ...
  function bookAppointmentSubmit(e) {
    e.preventDefault();
    ...
    if (isBooked) return showMessage("✗ This time slot is already booked", "error");
    ...
    setAllData((d) => ({ ...d, appointments: [...d.appointments, newAppointment] }));
    setDoctorDetailOpen(false);
  ...
  function cancelAppointment(apptId) {
    setAllData((d) => ({
      ...
      appointments: d.appointments.map((a) =>
        a.id === apptId ? { ...a, status: "cancelled", updatedAt: new Date().toISOString() } : a
      ),
    }));
  ...
  function rescheduleAppointment(apptId) {
    const appt = allData.appointments.find((a) => a.id === apptId);
    if (!appt) return;
    ...
    setAllData((d) => ({
      ...
      appointments: d.appointments.map((a) =>
        a.id === apptId
          ? { ...a, appointmentDate: newDate, appointmentTime: newTime, updatedAt: new Date().toISOString() }
          : a
      ),
    }));
      ...
      <nav className="sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            {page === "landing" && (
              <button onClick={openAuthSelector} className="btn-primary" style={{ fontSize: "0.9rem", padding: "10px 24px" }}>
                Get Started
              </button>
            )}
          ...
          <section id="features" style={{ padding: "80px 24px", position: "relative", zIndex: 10 }}>
            <div className="max-w-6xl mx-auto">
              <div style={{ textAlign: "center", marginBottom: 60 }}>
                <h2 style={{ marginBottom: 16 }}>Powerful Features</h2>
                <p style={{ color: "var(--text-secondary)", fontSize: "1.1rem", maxWidth: 500, margin: "0 auto" }}>
                  Experience cutting-edge healthcare technology
                </p>
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 32 }}>
                <div className="glass-card feature-card animate-fade-in-up animation-delay-1" style={{ padding: 32 }}>
                  <div style={{ width: 56, height: 56, background: "linear-gradient(135deg, rgba(0, 217, 255, 0.2), rgba(6, 182, 212, 0.2))", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, marginBottom: 16 }}>
                    👨‍⚕️
                  </div>
                  <h3 style={{ marginBottom: 12 }}>Board-Certified Doctors</h3>
                  <p style={{ color: "var(--text-secondary)", margin: 0, fontSize: "0.95rem" }}>Connect with verified physicians across 30+ specialties.</p>
                </div>

                <div className="glass-card feature-card animate-fade-in-up animation-delay-2" style={{ padding: 32 }}>
                  <div style={{ width: 56, height: 56, background: "linear-gradient(135deg, rgba(0, 217, 255, 0.2), rgba(6, 182, 212, 0.2))", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, marginBottom: 16 }}>
                    🔒
                  </div>
                  <h3 style={{ marginBottom: 12 }}>Bank-Level Security</h3>
                  <p style={{ color: "var(--text-secondary)", margin: 0, fontSize: "0.95rem" }}>Enterprise encryption & HIPAA compliance for privacy.</p>
                </div>

                <div className="glass-card feature-card animate-fade-in-up animation-delay-3" style={{ padding: 32 }}>
                  <div style={{ width: 56, height: 56, background: "linear-gradient(135deg, rgba(0, 217, 255, 0.2), rgba(6, 182, 212, 0.2))", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, marginBottom: 16 }}>
                    📅
                  </div>
                  <h3 style={{ marginBottom: 12 }}>Smart Scheduling</h3>
                  <p style={{ color: "var(--text-secondary)", margin: 0, fontSize: "0.95rem" }}>Real-time availability & instant appointment booking.</p>
                </div>
              </div>
            </div>
          </section>
      ...
      <div className={`modal-overlay ${authRoleModal ? "show" : ""}`}>
        <div className="glass" style={{ padding: 40, maxWidth: 400, width: "100%", animation: "fadeInUp 0.4s ease-out" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <button onClick={() => selectRole("patient")} className="glass-card" style={{ padding: 32, border: "2px solid rgba(0, 217, 255, 0.2)", cursor: "pointer", transition: "all 0.3s ease", textAlign: "center" }}>
            ...
            <button onClick={() => selectRole("doctor")} className="glass-card" style={{ padding: 32, border: "2px solid rgba(0, 217, 255, 0.2)", cursor: "pointer", transition: "all 0.3s ease", textAlign: "center" }}>
          ...
          <button onClick={closeAuthSelector} style={{ width: "100%", marginTop: 20, background: "none", border: "1px solid rgba(0, 217, 255, 0.2)", color: "var(--text-secondary)", padding: 12, borderRadius: 10, cursor: "pointer", fontWeight: 500 }}>
            Back
          </button>
        </div>
      </div>
                  ...
                  <div className="glass" style={{ padding: 40 }}>
                    ...
                    <form onSubmit={handlePatientRegisterSubmit}>
                      ...
                      <div style={{ marginBottom: 16 }}>
                        ...
                        <input className="input-field" type="tel" placeholder="(555) 000-0000" value={patientRegister.phone} onChange={(e) => setPatientRegister((s) => ({ ...s, phone: e.target.value }))} />
                      </div>
              ...
              <div className="glass-card" style={{ padding: 32 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24 }}>
                  {visibleDoctors.map((doc) => (
                    <div key={doc.id} className="glass-card" style={{ padding: 24, cursor: "pointer", transition: "all 0.3s ease" }} onClick={() => openDoctorDetails(doc.id)}>
                      <div style={{ fontSize: "2rem", marginBottom: 12 }}>⚕️</div>
                      <h4 style={{ margin: "0 0 8px 0", color: "white" }}>Dr. {doc.lastName}</h4>
                      <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", margin: "0 0 12px 0" }}>{doc.specialty}</p>
                      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 12 }}>
                        <span style={{ color: "#86efac", fontSize: "0.9rem" }}>⭐ 4.9</span>
                        <span style={{ color: "var(--text-secondary)", fontSize: "0.9rem" }}>{doc.yearsExperience}y exp.</span>
                      </div>
                      <button className="btn-primary" style={{ width: "100%", padding: 10, fontSize: "0.9rem" }} onClick={(ev) => { ev.stopPropagation(); openDoctorDetails(doc.id); }}>
                        Book Appointment
                      </button>
                    </div>
                  ))}
      ...
      {page === "doctor" && currentDoctor && (
        <div className="w-full h-full flex flex-col">
          ...
          <div className="tab-switch" style={{ maxWidth: 1400, margin: "32px auto", padding: "0 32px", width: "calc(100% - 64px)" }}>
            ...
            <button className={`tab-btn ${doctorTab === "schedule" ? "active" : ""}`} onClick={() => setDoctorTab("schedule")}>📊 Schedule</button>
          </div>
          ...
          <main className="flex-1 overflow-auto" style={{ padding: "0 32px 32px", maxWidth: 1400, margin: "0 auto", width: "100%" }}>
              ...
              <div className="glass-card" style={{ padding: 32 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24 }}>
                  ...
                  <div>
                    ...
                    <p style={{ color: "white", fontWeight: 600, margin: 0 }}>{currentDoctor.specialty}</p>
                  </div>
                  <div>
                    <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", margin: "0 0 8px 0" }}>License</p>
                    <p style={{ color: "white", fontWeight: 600, margin: 0 }}>{currentDoctor.licenseNumber}</p>
                  </div>
                  <div>
                    <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", margin: "0 0 8px 0" }}>Experience</p>
                    <p style={{ color: "white", fontWeight: 600, margin: 0 }}>{currentDoctor.yearsExperience} years</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="glass-card" style={{ padding: 32 }}>
                ...
                <div style={{ marginBottom: 32 }}>
                  <p style={{ color: "var(--text-secondary)", marginBottom: 16 }}>
                    Select the days you work and set your availability times:
                  </p>
                    ...
                    {[
                      ...
                      >
                        <input
                          ...
                          checked={workingDaysDraft.has(value)}
                          onChange={() => toggleWorkDay(value)}
                        />
                        <label style={{ color: "white", fontWeight: 500, margin: 0 }}>{label}</label>

                        {/* For simplicity: same start/end for all days (like your original saveWorkingHours used Monday) */}
                        <input
                          type="time"
                ...
                <div style={{ borderTop: "1px solid rgba(0, 217, 255, 0.1)", paddingTop: 32 }}>
                  <h3 style={{ marginTop: 0 }}>Available Time Slots This Week</h3>

                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 16, marginTop: 16 }}>
                    {doctorSlots.length === 0 ? (
                      <p style={{ color: "var(--text-secondary)" }}>
                        No available slots configured. Set your working hours first.
                      </p>
                    ) : (
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              textAlign: "center",
                              border: `1px solid ${
                                isBooked ? "rgba(239, 68, 68, 0.3)" : "rgba(0, 217, 255, 0.2)"
                              }`,
                            }}
                          >
                    ...
                    doctorAppointments.map((appt) => {
                      ...
                      return (
                        <div
                          ...
                          style={{
                            padding: 20,
                            borderLeft: `4px solid ${isUpcoming ? "var(--cyan-bright)" : "var(--text-secondary)"}`,
                          }}
                        >
            ...
            {doctorTab === "schedule" && (
              <div className="glass-card" style={{ padding: 32 }}>
                <h3 style={{ marginTop: 0, marginBottom: 24 }}>Weekly Schedule Overview</h3>

                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 16 }}>
                  {scheduleOverview.map((d) => (
                    <div
                      key={d.key}
                      className="glass-card"
                      style={{
                        padding: 20,
                        borderLeft: `4px solid ${d.isWorking ? "var(--cyan-bright)" : "rgba(156, 163, 175, 0.5)"}`,
                      }}
                    >
                      <h4 style={{ margin: "0 0 12px 0", color: "white" }}>{d.dayName}</h4>
                      <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", margin: "0 0 8px 0" }}>{d.dateLabel}</p>
                      <p style={{ color: "white", fontWeight: 600, margin: 0 }}>{d.hours}</p>
                      <p style={{ color: "var(--cyan-bright)", fontSize: "0.9rem", margin: "8px 0 0 0" }}>
      ...
      <div className={`modal-overlay ${editDoctorOpen ? "show" : ""}`} onClick={() => setEditDoctorOpen(false)}>
        <div className="glass" onClick={(e) => e.stopPropagation()} style={{ padding: 40, maxWidth: 500, width: "100%", animation: "fadeInUp 0.4s ease-out", maxHeight: "90vh", overflowY: "auto" }}>
          ...
          <form onSubmit={saveDoctorProfile}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontWeight: 600, color: "white", marginBottom: 6, fontSize: "0.85rem" }}>Specialty</label>
              <input className="input-field" value={doctorEditForm.specialty} disabled style={{ opacity: 0.6, cursor: "not-allowed" }} />
            </div>

            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontWeight: 600, color: "white", marginBottom: 6, fontSize: "0.85rem" }}>License Number</label>
              <input className="input-field" value={doctorEditForm.license} disabled style={{ opacity: 0.6, cursor: "not-allowed" }} />
            </div>
            ...
            <>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
                <div>
                  ...
                  <p style={{ color: "var(--cyan-bright)", fontWeight: 600, margin: 0 }}>{selectedDoctor.specialty}</p>
                </div>
              ...
              <div style={{ background: "rgba(0, 217, 255, 0.05)", padding: 16, borderRadius: 8, marginBottom: 24 }}>
                <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", margin: "0 0 8px 0" }}>Experience</p>
                <p style={{ color: "white", fontWeight: 600, margin: 0 }}>{selectedDoctor.yearsExperience} years</p>

                <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", margin: "12px 0 8px 0" }}>License</p>
                <p style={{ color: "white", fontWeight: 600, margin: 0 }}>{selectedDoctor.licenseNumber}</p>

                {selectedDoctor.bio ? (
                  <>
                    <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", margin: "12px 0 8px 0" }}>About</p>
                    <p style={{ color: "white", margin: 0 }}>{selectedDoctor.bio}</p>
                  </>
                ) : null}
              </div>
